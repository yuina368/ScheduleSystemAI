...
| Dashboard | 今日の予定、科目数制限、連続学習日数、優先度理由、最終達成確率、日別達成率、残り割合、ポモドーロタイマーを表示 |
...
Settings 画面の `テスト通知を送る` ボタンから、保存済み Webhook URL に即時送信できます。
...
Vercel Cron の時刻は UTC 基準です。`backend/vercel.json` では `0 21 * * *` を設定しており、日本時間の毎朝 6 時台に `/cron/morning-summary` を実行します。Hobby プランでは指定した時刻ちょうどではなく、その時間帯のどこかで実行されるため、7 時までに届きやすいように 6 時台へ寄せています。

通知が届かない場合は、まず Settings のテスト通知で Webhook URL を確認し、次に Backend Project の Vercel Cron Jobs ログで `/cron/morning-summary` の実行結果を確認します。Backend Project の環境変数 `CRON_SECRET` が未設定の場合、Cron エンドポイントは実行を拒否します。
